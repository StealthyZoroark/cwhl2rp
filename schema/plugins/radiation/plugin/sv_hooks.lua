...
function PLUGIN:ClockworkInitPostEntity() self:LoadRadiationAreas(); end;
...
function PLUGIN:Tick()
	if( !self.NextCheck or self.NextCheck < CurTime() )then
		...
		for k,v in pairs(_player.GetAll())do
			if(v:IsNoClipping() or !v:Alive())then continue; end;
			local plyInZone = false;
			local hasIncreased = false;
	
			for k2, v2 in pairs(self.radiationZones) do
				if (Clockwork.entity:IsInBox(v, v2.minimum, v2.maximum) ) then
					...
					-- Reduce the increased based on their mask, and reduce the filter quality. (Commented out because I'm too lazy to port the Gasmask plugin. If you have some kind of Gasmask system, just uncomment this.
					if ( HasMask ) then
